...
class Owner extends MX_Controller {
	...
	public function save_business() {
		...
		if(!empty($post)) {
				 ...
				 * -------------------- */
				$this->load->model('fees/owner_model');
				// if($this->owner_model->check_permit_no($post['permit_number'])){
				// 	response(1,'Existing Business Permit');
				// }else{
        //
				// }
				if($this->owner_model->save_business($post)) {
					...
					response(1, 'This business has been renewed this year');
				}
	...
	public function save_renew() {
		...
		$return  = $this->fees_model->save_renew($post);
		if($return['error'] == 0) {
			response(0, 'success');
		} else {
			response(1, $return['msg']);
		}
	}

	public function save_reqs() {
		$post = $this->input->post(null, true);
		$this->load->model('fees/fees_model');
		$return  = $this->fees_model->save_reqs($post);
		if($return['error'] == 0) {
			response(0, 'You were able to add the proper requirements for this applicant.');
		} else {
			response($return['error'], $return['msg']);
		}
	}
	...
	public function save_retire() {
		$post = $this->input->post(null, true);
		$this->load->model('fees/owner_model');
		if($this->owner_model->retire_now($post)) {
			response(0, 'Success');
		} else {
	...
	public function assess($appID = null) {
		if(!is_null($appID)) {
			...
			if(count($assessment) > 0 && !empty($assessment)) {
				...
				response(1, 'No Records found. Please check if this business has nature in the Edit section or check if the business has been renewed.');
			}
		} else {
			response(1, 'No owner id specified.');
		}
	...
	public function upload_image($buss_id = null){
		$post = $this->input->post(null, true);
       ...
       /* $config['max_size']             = 100;
		...
		$permit_no = explode(".",$_FILES['file']['name']);
       		...
       		if($this->upload->do_upload("file")){
       				...
       				if($this->owner_model->save_photo($config['upload_path'].$_FILES['file']['name'],$permit_no[0],$buss_id)){
	...
	public function upload_image1($buss_id = null){
		$post = $this->input->post(null, true);
		...
		$permit_no = explode(".",$_FILES['file']['name']);
       		...
       		if($this->upload->do_upload("file")){
       				...
       				if($this->owner_model->save_photo1($config['upload_path'].$_FILES['file']['name'],$permit_no[0],$buss_id)){
	...
	public function upload_image2($buss_id = null){
		$post = $this->input->post(null, true);
		...
		$permit_no = explode(".",$_FILES['file']['name']);
       		...
       		if($this->upload->do_upload("file")){
       				...
       				if($this->owner_model->save_photo2($config['upload_path'].$_FILES['file']['name'],$permit_no[0],$buss_id)){
	...
	public function upload_image3($buss_id = null){
		$post = $this->input->post(null, true);
		...
		$permit_no = explode(".",$_FILES['file']['name']);
       		...
       		if($this->upload->do_upload("file")){
       				...
       				if($this->owner_model->save_photo3($config['upload_path'].$_FILES['file']['name'],$permit_no[0],$buss_id)){
	...
	public function upload_image4($buss_id = null){
		$post = $this->input->post(null, true);
		...
		$permit_no = explode(".",$_FILES['file']['name']);
       		...
       		if($this->upload->do_upload("file")){
       				...
       				if($this->owner_model->save_photo4($config['upload_path'].$_FILES['file']['name'],$permit_no[0],$buss_id)){
	...
	public function delete_business_nature($appid = null, $natureid = null, $delinquentyear = null){
		...
		if(!is_null($appid) && !is_null($natureid)){
			$this->load->model('fees/owner_model');
			$result = $this->owner_model->delete_business_nature($appid,$natureid,$delinquentyear);
			if($result){
