...
// deleting announcement
	...
	public function permit_number(){
		$date=date('Y');
			return $this->db->update('businessess', array('permit_number' => $date.'-00000000'),'');
	...
	//$db_name = 'assets/database_backup/crm-on-' . date("Y-m-d-H-i-s") . '.zip';
	$db_name = 'assets/database_backup/crm-on.zip';
	...
	$this->zip->add_data('copy-'.$db_name.'.sql', $prefs['filename']);
	ob_end_clean();
	return $this->zip->download('dl-'.$db_name);

	}

	public function import_database()
	{
			$config['upload_path']          = './assets/database_backup/';
			$config['allowed_types']        = 'zip';
		  $this->load->library('upload', $config);  print_r($_FILES);
			$templine = '';
			$lines = 'assets/database_backup/crm-on.zip';
			print_r($this->upload());
